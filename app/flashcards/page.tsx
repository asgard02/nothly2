...
export default function CollectionsPage() {
  ...
  const handleCreateCollection = () => {
    if (selectedTags.length === 0) {
      setCreationError(t("dialog.noTags"))
      return
    }
  ...
  return (
    <div className="flex h-screen bg-background">
      <Sidebar />
      <main className="ml-56 flex-1 overflow-y-auto bg-muted/20">
              ...
              <DialogContent className="max-w-2xl">
                ...
                <div className="space-y-5 py-4">
                  ...
                  <div className="space-y-3">
                    <label className="text-sm font-medium text-muted-foreground">{t("dialog.tagsLabel")}</label>
                  ...
                  <div className="rounded-2xl border border-border/70 bg-muted/40 p-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-medium text-foreground">{t("dialog.contextTitle")}</p>
                        <p className="text-xs text-muted-foreground">
                          {estimatedTokens} {t("dialog.contextTokens", { chars: estimatedCharacters.toLocaleString(locale) })}
                        </p>
                      </div>
                      <span
                        className={cn(
                    ...
                    {contextOverLimit && (
                      <p className="mt-2 text-xs text-rose-600">
                        {t("dialog.contextOverLimit", { limit: CONTEXT_CHAR_LIMIT.toLocaleString(locale) })}
                      </p>
                    )}
                    {matchedDocuments.length > 0 && (
                      <div className="mt-4 space-y-1 text-xs text-muted-foreground">
                        <p className="font-medium text-foreground">{t("dialog.matchedDocuments")}</p>
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-muted-foreground flex items-center gap-1">
                          ...
                          {new Date(collection.created_at).toLocaleString(locale)}
                        </span>
                      </div>
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        {collection.tags?.length ? (
                          ...
                          <span className="text-xs text-muted-foreground/70">{t("card.noTags")}</span>
                        )}
                      </div>

                      <div className="mt-auto flex items-center justify-between pt-6 text-xs text-muted-foreground/80">
                        <span>{collection.total_flashcards} {t("card.flashcards")}</span>
                        <span>{collection.total_quiz} {t("card.questions")}</span>
                      </div>
                    </button>
                  </div>
                )
