        ...
        <div className="rounded-3xl bg-card border-2 border-border p-6 flex flex-col md:flex-row items-center justify-between shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] dark:shadow-[8px_8px_0px_0px_rgba(255,255,255,1)] gap-4">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            ...
            <div className="relative w-full md:w-72 group">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-foreground group-focus-within:text-foreground transition-colors" strokeWidth={3} />
              <input
                type="text"
        ...
        {filteredSubjects.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full relative z-10 px-4">
            <div className="relative z-10 flex flex-col items-center text-center w-full max-w-4xl mx-auto p-8 md:p-16 rounded-[2.5rem] border-2 border-border bg-card shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] dark:shadow-[12px_12px_0px_0px_rgba(255,255,255,1)] animate-in fade-in zoom-in-95 duration-500">
              ...
              <p className="text-muted-foreground font-medium text-lg md:text-xl max-w-2xl mx-auto mb-16 leading-relaxed">
              ...
              <div className="flex flex-col md:flex-row items-center justify-center gap-8 md:gap-12 mb-16 w-full">
                ...
                <ArrowRight className="hidden md:block h-6 w-6 text-foreground" strokeWidth={3} />
              ...
              <Button
                onClick={() => setIsCreateDialogOpen(true)}
                className="h-auto py-4 px-10 rounded-xl bg-foreground hover:bg-foreground/90 text-background text-lg font-black uppercase border-2 border-border shadow-[6px_6px_0px_0px_rgba(0,0,0,0.3)] dark:shadow-[6px_6px_0px_0px_rgba(255,255,255,0.3)] hover:translate-y-[2px] active:translate-y-[4px] active:shadow-none transition-all duration-300"
              >
        ...
        <div className="relative z-10 p-6 flex flex-col h-full justify-between">
          <div>
            <div className="flex justify-between items-start mb-4">
              <div className="p-3 rounded-xl bg-card border-2 border-border text-foreground group-hover:rotate-6 transition-all duration-300 shadow-sm">
                <FolderOpen className="h-6 w-6" strokeWidth={2.5} />
              </div>

              <button
                onClick={onDelete}
                className="p-2 rounded-lg bg-card border-2 border-border text-foreground hover:bg-destructive/10 hover:text-destructive transition-colors opacity-0 group-hover:opacity-100 shadow-sm"
              >
                <Trash2 className="h-4 w-4" />
              </button>
            </div>

            <h3 className="text-xl font-black text-foreground mb-2 line-clamp-1 leading-tight">
              {subject.title}
            </h3>

            <div className="flex items-center gap-3 text-xs text-foreground/70 font-bold uppercase">
              <span className="flex items-center gap-1.5 bg-card/50 px-2 py-1 rounded-md border border-border/10">
          ...
          <div className="mt-6 pt-4 border-t-2 border-border/10 flex items-center justify-between">
            <span className="text-xs text-foreground/60 font-black uppercase">{formatLastActive(subject.last_active)}</span>

            <div className="flex items-center gap-1 text-xs font-black text-foreground opacity-0 group-hover:opacity-100 transform translate-x-2 group-hover:translate-x-0 transition-all duration-300 uppercase">
